 ...
 */
- (FBRequestMSG*)openUrl:(NSString *)url
  ...
  [_request release];
  _request = [[FBRequestMSG getRequestWithParams:params
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) {
      ...
      NSString *fbAppUrl = [FBRequestMSG serializeURL:urlPrefix params:params];
    ...
    if (trySafariAuth && !didOpenOtherApp) {
      ...
      NSString *fbAppUrl = [FBRequestMSG serializeURL:loginDialogURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
  }
  ...
  if (!didOpenOtherApp) {
    [_loginDialog release];
    _loginDialog = [[FBLoginDialogMSG alloc] initWithURL:loginDialogURL
                                          loginParams:params
                                             delegate:self];
    [_loginDialog show];
  }
...
- (void)dialog:(NSString *)action
  ...
  if (action == kLogin) {
    ...
    _fbDialog = [[FBLoginDialogMSG alloc] initWithURL:dialogURL loginParams:params delegate:self];
  } else {
    ...
    _fbDialog = [[FBDialogMSG alloc] initWithURL:dialogURL params:params delegate:delegate];
  }
